...
//*************************************************************************************************************
	void autoPlay()
	{
		while(teamOne.getSize() > 1 && teamTwo.getSize() > 1)				//continue play until there is a winner
		{
			...
			cout << "       _" << endl << "     _( }" << endl << " _  <<  \\" << endl;
			cout << "`.\\__/`/\\\\" << endl << "  \'--\'\\\\" << endl << "     //" << endl << "     \\)" << endl;
			cout << "\t\t" << runner << " running at " << at << endl;
//-----------------------------------------------------------------------------------------------------------------------------------------------
			//TEAM ONE RUNNING
			if(teamTwo.tailCase(at))					//running at the tail
			{
				...
				{
					cout << "\t\t" << runner << " makes it through!" << endl;
					...
					{
						cout << " ____      ____   _____    ___   __   ___   __    ______   ______" << endl;
						cout << "|_  _|    |_  _| |_   _|  |   \\ |  | |   \\ |  |  |   _  | |   _  |" << endl;
						cout << "  \\ \\  /\\  / /     | |    |  \\ \\|  | |  \\ \\|  |  |   ___| |  | |_|" << endl;
						cout << "   \\ \\/  \\/ /     _| |_   |  |\\ |  | |  |\\ |  |  |  |___  |  |" << endl;
						cout << "    \\__/\\__/     |_____|  |__| \\___| |__| \\___|  |______| |__|" << endl;
						cout << "\nWinner: Team 1!\n";
						return;
					}
				}
...
//*************************************************************************************************************
	void shuffle()					//randomly reinserts random players
	{
		for(int i = 0; i < (2 * roster.getSize()); i++)//number of shuffles will be equal to twice the size of the roster
		{
			...
			int strength = roster.getStrength(shuff);
			int speed = roster.getSpeed(shuff);
			roster.erase(shuff);					//remove the player to be reinserted
			roster.taken(shuff, after, strength, speed);	//insert the player after the insertion player
		}
	}
